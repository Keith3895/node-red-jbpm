...
module.exports = function (RED) {
    function listTasksNode(config) {
        ...
        this.on('input', function (msg) {
            ...
            var pageSize = msg.pageSize || config.pageSize || 10;
            var sortOrder = msg.sortOrder || config.sortOrder || true;
            var user = msg.Filter_user_id || config.Filter_user_id;
            if (validator.isURL(config.url) && !validator.isEmpty(status) && !validator.isEmpty(groups) && !validator.isEmpty(user_id) && !validator.isEmpty(password)) {
                ...
                var options = {
                    method: 'GET',
                    // pot-owners?status=Ready&groups=telesureAgents&page=0&pageSize=10&sortOrder=true
                    uri: config.url + '/queries/tasks/instances/pot-owners?status='+status+'&groups='+groups+'&user='+user+'&page='+page+'&pageSize='+pageSize+'&sortOrder='+sortOrder,
                    headers: {
