    ...
    function checkOutNode(config) {
        ...
        this.on('input', function (msg) {
            ...
            if (!validator.isEmpty(auth_header) && !validator.isEmpty(url) && !validator.isEmpty(task_id)) {
                if (claim) {
                    claimFunction(url,content_type, auth_header, task_id, container_name, msg);
                }
                if (start) {
                    startFunction(url,content_type, auth_header, task_id, container_name, msg);
                }
                if (!claim && !start) {
        ...
        function claimFunction(url,content_type, auth_header, task_id, container_name, msg) {
            ...
            url = urlPack.parse(url + '/containers/' + container_name + '/tasks/' + task_id + '/states/claimed');
        ...
        function startFunction(url,content_type, auth_header, task_id, container_name, msg) {
            ...
            url = urlPack.parse(url + '/containers/' + container_name + '/tasks/' + task_id + '/states/started');
            options = Object.assign(options, url);
            var req = http.request(options, function (res) {
                var buffer = "";
                res.on("data", function (data) { buffer = buffer + data; });
