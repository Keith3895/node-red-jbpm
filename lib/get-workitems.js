...
module.exports = function (RED) {
    function getWorkItems(config) {
        ...
        this.on('input', function (msg) {
            ...
            if (validator.isURL(config.url) && !validator.isEmpty(container_name) && !validator.isEmpty(process_instance_id) && !validator.isEmpty(auth_header) ) {
                ...
                // console.log(config.url + '/containers/'+container_name+'/processes/instances/'+process_instance_id+'workitems' );
                var options = {
                    method: 'GET',
                    uri: config.url + '/containers/'+container_name+'/processes/instances/'+process_instance_id+'/workitems' ,
                    headers: headers
                }
    ...
    RED.nodes.registerType('get-workitems', getWorkItems);
}
