...
module.exports = function (RED) {
    function startProcessNaode(config) {
        ...
        node.on('input', function (msg) {
            ...
            let process_name = msg.process_name || config.process_name;
            let auth_header = msg.auth_header || new Buffer(user_id + ':' + password).toString('base64');
            if (!validator.isEmpty(auth_header) && validator.isURL(url) && !validator.isEmpty(config.container_name) && !validator.isEmpty(process_name)) {
                show_status(node, 'in_progress')
                url = remove_slash(url)
                var options = {
                    method: 'POST',
                    uri: url + '/containers/' + container_name + '/processes/' + process_name + '/instances/',
                    body: msg.payload,
                    headers: {
