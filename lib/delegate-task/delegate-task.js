...
module.exports = function (RED) {
    function delgateTask(config) {
        ...
        this.on('input', function (msg) {
            ...
            let targetOwner = msg.targetOwner || config.targetOwner;
            if (!validator.isEmpty(auth_header) && !validator.isEmpty(url) && !validator.isEmpty(task_id) && !validator.isEmpty(targetOwner)) {
                ...
                url += '/containers/' + container_name + '/tasks/' + task_id + '/states/delegated?targetUser='+targetOwner;
                ...
                showMessage(500, 'Internal server error', 'Please check authorisation and url and task_id and targetOwner', 'error', 'Please check authorisation and url and task_id and targetOwner', msg, node);
            }
        });

    }
    RED.nodes.registerType('delegate-task', delgateTask);
}