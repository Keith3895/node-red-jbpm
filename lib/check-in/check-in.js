...
module.exports = function (RED) {
    function checkInNode(config) {
        ...
        this.on('input', function (msg) {
            ...
            if (!validator.isEmpty(auth_header) && !validator.isEmpty(url) && !validator.isEmpty(task_id)) {
                ...
                if (save && !complete && msg.payload) {
                    saveFunction(url, content_type, auth_header, msg.payload, task_id, container_name, onwer, msg);
                } else if (!save && complete) {
                    completeFunction(url, content_type, auth_header, {}, task_id, container_name, onwer, msg);

                } else if (save && complete && msg.payload) {

                    completeFunction(url, content_type, auth_header, msg.payload, task_id, container_name, onwer, msg);
                } else {
        ...
        function saveFunction(url, content_type, auth_header, payload, task_id, container_name, onwer, msg) {
        ...
        function completeFunction(url, content_type, auth_header, payload, task_id, container_name, onwer, msg) {
            var headers = {
                'Authorization': auth_header,
                'Content-Type': content_type,
            };
